...
const MarkOut = () => {
  ...
  const handleMarkOut = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/mark-out', {
        method: 'POST',
      ...
      if (data.success) {
        setResult({ type: 'success', message: `Marked Out as ${data.username} at ${data.time}` });
      } else {
  ...
  return (
    ...
    >
      <div className="bg-black bg-opacity-80 p-6 rounded-xl shadow-lg text-white text-center max-w-xl w-full">
        <h2 className="text-2xl font-bold mb-4">Mark Out</h2>
        <Webcam
          audio={false}
          ref={webcamRef}
        ...
        <button
          ...
          className={`px-4 py-2 rounded font-semibold transition ${
            marking ? 'bg-gray-600 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'
          }`}
        >
          {marking ? 'Marking...' : 'Mark Out'}
        </button>
